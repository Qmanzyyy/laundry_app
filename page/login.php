...
if (isset($_POST["kirim"])) {
    ...
    if (mysqli_num_rows($result) === 1) {
        ...
        if ($password == $row["password"]) {
            ...
            $_SESSION['user_role'] = $row["role"]; //simpan role pengguna dalam session
...
<body class="h-screen flex justify-center items-center bg-blue-50">
  <form action="" method="post" class="bg-white shadow-[0_4px_10px_rgba(33,28,132,1)] rounded-lg border-2 border-[#211C84]">
    <div class="w-64 p-6 text-center">
      ...
      <button name="kirim" class="active:bg-[#211C84] focus:outline-2 focus:outline-offset-2 focus:outline-violet-500 hover:bg-[#4D55CC] border text-white w-full bg-[#7A73D1] p-2 mt-6 rounded-lg" type="submit">
        Login
      </button>
    </div>
  </form>
