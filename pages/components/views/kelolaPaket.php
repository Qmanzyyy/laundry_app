...
<main class="py-10 bg-gray-50 min-h-screen">
  <div class="container mx-auto px-4">
    <h1 class="text-3xl font-bold text-center text-gray-800 mb-10">Kelola Paket</h1>
    <button class="bg-indigo-600 text-white p-2 font-bold rounded-lg mb-3"><a href="?tab=kelolaProdukPaket">Kelola Jenis Cuci</a></button>
      ...
      <table class="min-w-full divide-y divide-gray-200 text-sm">
        <thead class="bg-gray-100">
          <tr>
            ...
            <th class="px-6 py-3 text-left font-semibold text-gray-600">Harga Paket</th>
        ...
        <tbody class="divide-y divide-gray-100">
          <?php $no = 1; foreach ($dataTransaksi as $row): ?>
            <tr class="hover:bg-blue-50 transition">
              ...
              <td class="px-6 py-4"><?= $row['paket_cuci'] ?></td>
              <td class="px-6 py-4">Rp<?= number_format($row['harga_paket'], 0, ',', '.') ?></td>
              <td class="px-6 py-4 text-center flex justify-center gap-2 flex-wrap">
                <button
                  onclick="openEditJensiCuciModal(this)"
                  data-id="<?= $row['id'] ?>"
                  data-nama="<?= htmlspecialchars($row['paket_cuci']) ?>"
                  data-alamat="<?= htmlspecialchars($row['harga_paket']) ?>"
  ...
  <div class="bg-white p-6 rounded-xl shadow-xl w-full max-w-md relative animate-fade-in">
    ...
    <form id="editForm" action="" method="POST" class="space-y-4">
      ...
      <div>
        ...
        <input type="text" name="paket_cuci" id="edit_nama" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring focus:ring-blue-300 focus:outline-none">
      </div>
      <div>
        <label class="block text-gray-600 font-medium mb-1">Harga</label>
        <input type="number" name="harga_paket" id="edit_alamat" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring focus:ring-blue-300 focus:outline-none">
      </div>
      <div class="flex justify-end gap-3 pt-2">
        ...
        <button type="submit" name="updateJenisPaket" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition">Simpan</button>
      </div>
    </form>
  </div>
</div>
